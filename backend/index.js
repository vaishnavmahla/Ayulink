...
const http = require('http'); // 1. Import raw HTTP
...
const server = http.createServer(app); // <--- DEFINES THE SERVER!
...
// --- BOOKING ENGINE (BULLETPROOF DOUBLE-BOOKING BLOCKER) ---
app.post('/api/appointments', authenticateToken, async (req, res) => {
  try {
    ...
    if (!patient) return res.status(404).json({ error: "Patient profile not found. Please log in again." });
    ...
    if (existingAppointment) {
      return res.status(400).json({ 
        error: "Slot unavailable! Another patient has already claimed this time." 
      });
    }
    ...
    io.emit("new_booking_alert", {
      doctorId: parseInt(doctorId),
      message: "New patient in the waiting room!"
    });
...
app.put('/api/doctor/appointments/:appointmentId/approve', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ message: "Appointment officially approved!", appointment: updatedAppointment });
  } catch (error) {
...
app.get('/api/setup-patient', async (req, res) => {
  try {
    ...
    const patient = await prisma.user.upsert({
      where: { email: "suresh.raina@example.com" },
    ...
    res.json({ message: "✅ PATIENT ACCOUNT CREATED SUCCESSFULLY!", email: "suresh.raina@example.com", password: "password123" });
  } catch (error) {
...
app.get('/api/setup-doctor', async (req, res) => {
  try {
    ...
    res.json({ message: "✅ DOCTOR ACCOUNT CREATED SUCCESSFULLY!", email: "doctor@ayulink.com", password: "doctor123" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
server.listen(PORT, () => {  // <--- Changed 'app' to 'server'
