...
const DoctorDashboard = () => {
  ...
  const handleNoShow = async (appointmentId) => {
    try {
      // Tell the database to cancel this appointment
      await axios.put(`http://localhost:3000/api/doctor/appointments/${appointmentId}/reject`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });

      // Remove them from the local UI queue immediately
      setAppointments(prev => prev.filter(apt => apt.id !== appointmentId));

      // If the no-show patient was currently selected on the right, clear the screen
  ...
  // Calculate the active patients so we can use this for the badge AND the list
  const activeQueue = appointments.filter(apt => apt.status === 'PENDING' || apt.status === 'APPROVED');
      ...
      <main className="max-w-7xl mx-auto p-8 relative">
        ...
        <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-4 py-2 rounded-xl font-bold flex items-center gap-2">
            <Users size={18} /> {activeQueue.length} Patients in Queue
          </div>
        </header>
              ...
              {loading ? (
                ...
                activeQueue.map((apt) => (
                  <div
                    ...
                    className={`relative group cursor-pointer w-full text-left p-5 rounded-2xl border transition-all flex justify-between items-center gap-4 overflow-hidden
    ${selectedPatient?.id === apt.id ? 'bg-slate-900/80 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.1)]' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}
                  >
                    ...
                    <div>
                      <p className="font-bold text-slate-100 text-lg">{apt.patient.user.name}</p>
                    ...
                    <div className="flex flex-col items-end gap-3">
                      ...
                      {/* TOP ROW: Time & Status Badges (Side-by-Side) */}
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold bg-slate-800 text-slate-300 px-2.5 py-1 rounded-md flex items-center gap-1.5 border border-slate-700/50">
                          <Clock size={12} /> {apt.time}
                        </span>

                        {apt.status === 'PENDING' ? (
                          <span className="text-[10px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-400 px-2 py-1 rounded-md border border-amber-500/30 animate-pulse">
                            Pending
                          </span>
                        ) : apt.status === 'APPROVED' ? (
                          <span className="text-[10px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-md border border-emerald-500/30">
                            Approved
                          </span>
                        ) : (
                          <span className="text-[10px] font-black uppercase tracking-wider bg-red-500/10 text-red-400 px-2 py-1 rounded-md border border-red-500/30">
                            Cancelled
                          </span>
                        )}
                      </div>
                      ...
                      <div className="h-[28px] flex items-center justify-end">
                        {apt.status === 'PENDING' && (
                          <div className="hidden group-hover:flex gap-2">
                            <button
                              ...
                              className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-black text-[10px] uppercase tracking-wider px-3 py-1.5 rounded-md shadow-lg transition-all"
                            >
                              Approve
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleRejectAppointment(apt.id); }}
                              className="bg-red-500 hover:bg-red-400 text-white font-black text-[10px] uppercase tracking-wider px-3 py-1.5 rounded-md shadow-lg transition-all"
                            >
                              Reject
                            </button>
                          </div>
                        ...
                        {apt.status === 'APPROVED' && (
                          <button
                            ...
                            className="text-[10px] font-bold uppercase tracking-wider text-slate-400 hover:text-red-400 bg-slate-900/50 hover:bg-red-500/10 border border-slate-700/50 hover:border-red-500/30 px-3 py-1.5 rounded-md transition-all"
                          >
                            Mark No-Show
                          </button>
                        )}
